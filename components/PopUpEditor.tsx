...
const PopUpEditor = ({
  ...
  const [updatetedStudent, setUpdatedStudent] = useState<any>();
  const data = useSelector((state: any) => state.student.studentData);
  ...
  const handleUpdate = (e: any) => {
    e.preventDefault();
    if (!student.rollnumber == updatetedStudent?.rollnumber) {
      if (
        data.some((std: any) => std.rollnumber === updatetedStudent?.rollnumber)
      ) {
        setIsDuplicate(true);
        return;
      }
    ...
    let newData = data?.map((std: any) => {
      if (std.rollnumber == student.rollnumber) {
        return {
          rollnumber: updatetedStudent.rollnumber || std.rollnumber,
          name: updatetedStudent.name || std.name,
          degree: updatetedStudent.degree || std.degree,
          semester: updatetedStudent.semester || std.semester,
        };
      }
      return std;
    });
    ...
    <div className="fixed bg-opacity-40 inset-0 flex justify-center items-center h-screen bg-black">
      <div className="flex flex-col font-mono py-4 rounded-lg gap-8 w-2/6 px-4 bg-gray-50">
        <div className="flex justify-between items-center px-2">
          ...
          <p onClick={closePopup} className="text-xl font-bold cursor-pointer">
            X
          </p>
        </div>
        <form onSubmit={handleUpdate}>
          ...
          <div>
            ...
            <input
              ...
              name="rollnumber"
              onChange={(e: any) => {
                if (student.rollnumber === e.target.value) {
                  setUpdatedStudent((params: any) => (params.rollnumber = ""));
                  return;
                }
                setUpdatedStudent({
                  ...updatetedStudent,
